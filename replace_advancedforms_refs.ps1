...
$projects = @(
    ...
    "SimPE.EnumComboBox\SimPE.EnumComboBox.csproj",
...
foreach ($proj in $projects) {
    ...
    $content = [regex]::Replace($content, '<Reference Include="AdvancedForms"[^>]*/>', '', [System.Text.RegularExpressions.RegexOptions]::Singleline)
    $content = [regex]::Replace($content, '<Reference Include="AdvancedForms"[^>]*>.*?</Reference>', '', [System.Text.RegularExpressions.RegexOptions]::Singleline)

    if ($content -notmatch 'AdvancedForms\.csproj') {
        $content = $content -replace '</Project>', "  <ItemGroup>`r`n    <ProjectReference Include=""..\AdvancedForms\AdvancedForms.csproj"" />`r`n  </ItemGroup>`r`n</Project>"
    }
