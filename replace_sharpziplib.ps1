...
foreach ($proj in $projects) {
    ...
    $content = [regex]::Replace($content, '<Reference Include="ICSharpCode\.SharpZipLib"[^>]*/>', '', [System.Text.RegularExpressions.RegexOptions]::Singleline)
    $content = [regex]::Replace($content, '<Reference Include="ICSharpCode\.SharpZipLib"[^>]*>.*?</Reference>', '', [System.Text.RegularExpressions.RegexOptions]::Singleline)

    if ($content -ne $before) {
        Set-Content $path $content -Encoding UTF8 -NoNewline
        Write-Host "Removed DLL ref: $proj"
    }
}
